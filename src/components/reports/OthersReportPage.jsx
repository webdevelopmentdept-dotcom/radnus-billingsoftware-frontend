...
const OthersReportPage = () => {
  ...
  const processData = (jobsheets) => {
    ...
    jobsheets.forEach((item) => {
      ...
      if (othersItems.length > 0) {
        othersItems.forEach((oi) => {
          ...
          if (amt > 0) {
            ...
            entries.push({ date: d, category: oi.category || "-", amount: amt });
          }
        });
      } else {
        const amt = Number(item.service?.othersAmount || 0);
        if (amt > 0) entries.push({ date: repairDate, category: "-", amount: amt });
      }
      ...
      entries.forEach((e) => {
        ...
        grouped[e.date].push({ jobSheetNo, name, category: e.category, amount: e.amount });
        gTotal += e.amount;
      });
    });
  ...
  const handleExcel = () => {
    ...
    Object.entries(groupedData).forEach(([date, records]) => {
      records.forEach((item, i) => {
        rows.push({ "Date": date, "SL No": i + 1, "Job Sheet": item.jobSheetNo, "Customer": item.name, "Category": item.category, "Amount": item.amount });
      });
    });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">📦 Others Expense Report</h1>
        <p className="text-sm text-gray-500">Date-wise other expenses</p>
      </div>
